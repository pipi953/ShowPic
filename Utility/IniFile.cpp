...
BOOL CIniFile::OpenIniFile (CCHR * FileName)
{
	...
	while (fgets (Str, 1024, m_pIniFile) != NULL)
	{
		pStr = strchr (Str, '\n');
		if (pStr != NULL) { *pStr = 0; }
		pEntry = MakeNewEntry ();
...
*========================================================================*/
void CIniFile::WriteFileString (CCHR *pSection, CCHR *pKey, CCHR *pValue)
{
	EFIND List;
	char  Str [4096];
...
*========================================================================*/
void CIniFile::WriteFileBool (CCHR *pSection, CCHR *pKey, bool Value)
{
	char Val [64] = {'0',0};
	if (Value != 0) { Val [0] = '1'; }
	WriteFileString (pSection, pKey, Val);
}

/*=========================================================================
   WriteInt : Writes an integer to the ini file
*========================================================================*/
void CIniFile::WriteFileInt (CCHR *pSection, CCHR *pKey, int Value)
{
	char Val [64]; /* 32bit maximum + sign + \0 */
	sprintf (Val, "%d", Value);
	WriteFileString (pSection, pKey, Val);
}

/*=========================================================================
   WriteFloat : Writes a float to the ini file
*========================================================================*/
void CIniFile::WriteFileFloat (CCHR *pSection, CCHR *pKey, float Value)
{
	char Val [64]; 
	sprintf (Val, "%.12f", Value);
	WriteFileString (pSection, pKey, Val);
}

void CIniFile::WriteFileFloatArray (CCHR *pSection, CCHR *pKey, float* ValueArray, int arrayNum)
{
	char Val [4096];

	sprintf (Val, "%.12f", ValueArray[0]);

	for (int dIndex = 1; dIndex < arrayNum; dIndex++)
		sprintf (Val, "%s %.12f", Val, ValueArray[dIndex]);
...
*========================================================================*/
void CIniFile::WriteFileDouble (CCHR *pSection, CCHR *pKey, double Value)
{
	char Val [64]; /* DDDDDDDDDDDDDDD+E308\0 */
...
*========================================================================*/
CCHR *CIniFile::ReadFileString (CCHR *pSection, CCHR *pKey, CCHR *pDefault)
{
	EFIND List;
...
*========================================================================*/
BOOL CIniFile::ReadFileBool (CCHR *pSection, CCHR *pKey, BOOL Default)
{
	char Val [64] = {"0"};
	if (Default != 0) { Val [0] = '1'; }
	return (atoi (ReadFileString (pSection, pKey, Val))?1:0); /* Only allow 0 or 1 */
}

/*=========================================================================
   ReadInt : Reads a integer from the ini file
*========================================================================*/
int CIniFile::ReadFileInt (CCHR *pSection, CCHR *pKey, int Default)
{
	char Val [64];
	sprintf (Val,"%d", Default);
	return (atoi (ReadFileString (pSection, pKey, Val)));
}

/*=========================================================================
   ReadFloat : Reads a float from the ini file
*========================================================================*/
float CIniFile::ReadFileFloat(CCHR *pSection, CCHR *pKey, float Default)
{
	char Val[64];
	sprintf (Val, "%.12f", Default);

	return (atof(ReadFileString (pSection, pKey, Val)));
}

void CIniFile::ReadFileFloatArray(CCHR *pSection, CCHR *pKey, float* valueArray, int arrayNum)
{
	char buffer[64] = {'\0'}; int dIndex = 0, bufferIndex = 0, charIndex = 0;
	
	ReadFileString(pSection, pKey, m_result);

	//以间隙空格作为读取文件信息的分割符
	for (dIndex = 0, bufferIndex = 0, charIndex = 0; dIndex < arrayNum - 1; dIndex++)
	{
		memset(buffer, '\0', 64);

		while(true)
		{
			if(m_result[charIndex] == 0x20)  //表示为空格
			{
				bufferIndex = 0; charIndex++;
				break;
			}

			buffer[bufferIndex++] = m_result[charIndex++];
		}

		valueArray[dIndex] = atof(buffer);
	}
}
...
*========================================================================*/
double CIniFile::ReadFileDouble (CCHR *pSection, CCHR *pKey, double Default)
{
	double Val;
	sprintf (m_result, "%1.10lE", Default);
	sscanf (ReadFileString (pSection, pKey, m_result), "%lE", &Val);
	return Val;
}
